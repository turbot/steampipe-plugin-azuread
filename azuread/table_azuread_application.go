...
func listAdApplications(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	err = pageIterator.Iterate(ctx, func(pageItem models.Applicationable) bool {
		isAuthorizationServiceEnabled := pageItem.GetAdditionalData()["isAuthorizationServiceEnabled"]

		d.StreamListItem(ctx, &ADApplicationInfo{pageItem, isAuthorizationServiceEnabled})
...
func getAdApplicationOwners(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	requestParameters := &applications.ItemOwnersRequestBuilderGetQueryParameters{
		Count: Bool(true),
	}

	config := &applications.ItemOwnersRequestBuilderGetRequestConfiguration{
		Headers:         headers,
		QueryParameters: requestParameters,
	}

	ownerIds := []*string{}
	owners, err := client.Applications().ByApplicationId(*applicationID).Owners().Get(ctx, config)
	if err != nil {
		errObj := getErrorObject(err)
